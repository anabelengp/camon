...
package com.shortybmc.data.parser
{
	...
	public class CSV extends URLLoader
	{
		...
		private function fieldDetection(element:*,index:int,arr:Array):void {
			...
			var trimArray:Array=new Array(fieldEnclosureToken,' ','\t');
			// loop through all record sets, checking for fieldSeperator (,) contained within fieldEnclosureToken (quotes)
			// and these separators are not considered real separators and fields aren't split there.
			for (var i:uint=0; i<tmp.length; i++) {
				if (! Boolean(count%2)) {
					result.push(StringUtils.trim(tmp[i],trimArray));
				} else {
					result[result.length-1]+=fieldSeperator+StringUtils.trim(tmp[i],trimArray);
				}
